...
def latest_vehicle_status_v3(request):
    ...
    f = open("/home/mcgm/Development/mcgm/mcgm/dashboard/timelog.txt", "a")
    ...
    for line in a_file:
        ...
        mydict[key] = value
    #ajit code block 1 ends here
    f.write("\nTime taken to initialize address dictionary was %s seconds" % (time.time() - start_time))
    ...
    if iward:
        ...
        #print(all_vth_record)
        #vehicles  = list(map(lambda vehicle : Vehicle.objects.get(pk=vehicle) , form_data['selectVehicle'].split("_")))
        f.write("\nTime taken to retrieve data for ward officer/contractor from database was %s seconds" % (time.time() - start_time))
        return render(request,'dashboard/latest_vehicle_status.html',{ 'all_online_entries' : response_data['data']['online'],'all_offline_entries' : response_data['data']['offline'] , 'scvcount' : scvcount, 'lccount' : lccount, 'mccount' : mccount, 'swcount' : swcount, 'dwcount' : dwcount, 'wwcount': wwcount, 'othercount' : othercount,'totalvehiclecount' : totalcount })
        ...
        file_eachrecord = open("/home/mcgm/Development/mcgm/mcgm/dashboard/tempdb.txt", "w", encoding="utf-8")
    ...
    #vehicles  = list(map(lambda vehicle : Vehicle.objects.get(pk=vehicle) , form_data['selectVehicle'].split("_")))
        ...
        """ file_eachrecord = open("/home/mcgm/Development/mcgm/mcgm/dashboard/tempdb2.txt", "w", encoding="utf-8")
        for each_th_record_dict in Current_tracklog_history.objects.values('vehicle').annotate(max_dt=Max('datetime')).order_by('max_dt'):
            each_th_record = Current_tracklog_history.objects.get(datetime=each_th_record_dict['max_dt'],vehicle_id=each_th_record_dict['vehicle'])
            each_vehicle_record_data = dict()
            each_vehicle_record_data['veh'] = str(each_th_record.vehicle)
    ...
    #print(all_vth_record)
    #vehicles  = list(map(lambda vehicle : Vehicle.objects.get(pk=vehicle) , form_data['selectVehicle'].split("_")))
        file_eachrecord.write(str(response_data2))
        file_eachrecord.close() """
        ...
        return render(request,'dashboard/latest_vehicle_status.html',{ 'all_online_entries' : response_data['data']['online'],'all_offline_entries' : response_data['data']['offline'] , 'scvcount' : '449', 'lccount' : '287', 'mccount' : '204', 'swcount' : '0', 'dwcount' : '38', 'wwcount': '30', 'othercount' : '39','totalvehiclecount' : '1047' })
...
def create_response_data_for_superuser():
    ...
    file_eachrecord = open("/home/mcgm/Development/mcgm/mcgm/dashboard/tempdb2.txt", "a", encoding="utf-8")
    ...
    #print(all_vth_record)
    #vehicles  = list(map(lambda vehicle : Vehicle.objects.get(pk=vehicle) , form_data['selectVehicle'].split("_")))
    file_eachrecord.write("The data is:"+ str(each_th_record_dict))
    file_eachrecord.close()
