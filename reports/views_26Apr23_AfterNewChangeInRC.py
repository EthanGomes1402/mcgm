...
def my_view(request):
    ...
    if request.method == 'POST':
        ...
        each_vehicle_record_data['plate_number'] = list()
        each_vehicle_record_data['distance'] = list()
        each_vehicle_record_data['vehicle_type'] = list()
        each_vehicle_record_data['date'] = list()
        each_vehicle_record_data['shift'] = list()
        each_vehicle_record_data['route_code'] = list()
        each_vehicle_record_data['route_name'] = list()
        each_vehicle_record_data['length'] = list()
        
        date = request.POST['date1']
        shift = request.POST['shift']
        
        each_vehicle_record_data['date'].append(date)
        each_vehicle_record_data['shift'].append(shift)
    ...
    #print("WARD ID IS :",fetch0[0])
        ...
        postgreSQL_select_Query1 = "select t1.id,t2.route_code,t2.route_name from swm.vehicles t1, swm.route_allocation t2 where t1.ward_id = '"+ward_id+"' and t1.vehicle_type IN ('Refuse_Compactor','Refuse_Mini_Compactor','Small_Closed_Vehicle') AND t1.id = t2.vehicle_id AND t2.is_active = 't';"
    #select1 = (ward_id)
        cursor.execute(postgreSQL_select_Query1)
    ...
    #print("This is fetch 1 ", fetch1)
        ...
        for row in fetch1:
            ...
            route_code_array.append(route_code)                         #%%%%%%%%%%%%%
            each_vehicle_record_data['route_code'].append(route_code)
            
            route_name_array.append(route_name)                         #%%%%%%%%%%%%%
            each_vehicle_record_data['route_name'].append(route_name)
        ...
        for all_vehicle_id in vehicle_id_array: 
            ...
            for row2 in fetch2:
                ...
                plate_number.strip()
                plate_number_store.append(plate_number)            #$$$$$$$$$$
                each_vehicle_record_data['plate_number'].append(plate_number)
                
                #print("plate number is ",plate_number) 
                vehicle_type_store.append(vehicle_type)            #$$$$$$$$$$
                each_vehicle_record_data['vehicle_type'].append(vehicle_type)
            ...
            total_distance_in_km = str(total_distance_in_km)
            distance_store.append(total_distance_in_km)                       #%%%%%%%%%%%%%%%%
            each_vehicle_record_data['distance'].append(total_distance_in_km) 
    ...
    trial_res['data'].append(each_vehicle_record_data)
    return render(request,'reports/test_template.html',{'trial_res' : trial_res['data']})
##done
...
def route_compliance(request):
    ...
    #print("Todate ", todate)    
        ...
        for row1 in fetch1:
            ...
            postgreSQL_select_Query2 = f"select longitude,latitude,datetime from swm.vehicle_tracklog_historys where vehicle_id = '{vehicle_id}' and datetime between '{fromdate}' and '{todate}' and shift = '{shift}' order by datetime asc;"
            ...
            postgreSQL_select_Query3 = f"select bin_location,bin_code from swm.route_compliance where route_code_id = '{route_code_id}' and is_active = 't';"
