...
def distance_report(request):
    ...
    if request.method == 'POST':
        ...
        postgreSQL_select_Query0 = "select id from common_ward where name = %s"
        select0 = (iward)
        cursor.execute(postgreSQL_select_Query0,select0)
        fetch0 = cursor.fetchall()
        for row0 in fetch0:
            ward_id = row0[0]

    
        postgreSQL_select_Query1 = "select id from swm.vehicles where ward_id = '%s' and vehicle_type IN ('Refuse_Compactor','Refuse_Mini_Compactor','Small_Closed_Vehicle')"
        select1 = (ward_id)
        cursor.execute(postgreSQL_select_Query1,select1)
        ...
        for all_vehicle_id in range(len(vehicle_id_array)):
            postgreSQL_select_Query2 = "select vehicle_type,plate_number from swm.vehicles where id = %s"
            select2 = (all_vehicle_id)
            cursor.execute(postgreSQL_select_Query2,select2)
            ...
            postgreSQL_select_Query3 = "select latitude,longitude from swm.vehicle_tracklog_historys where datetime between '%s' and '%s' and shift = '%s' and vehicle_id = '%s' order by datetime asc;"
            select3 = (fromdate,todate,shift,all_vehicle_id)
            cursor.execute(postgreSQL_select_Query3,select3)
            ...
            for i in range(len(location_array) - 2):
                
                source_location = location_array[i]
                destination_location = location_array[i+1]

                postgreSQL_select_Query4 = "select (point(%s) <@> point(%s)) as distance;"
                select4 = (source_location,destination_location)
            ...
            for single_distance in range(0, len(distance_array)):
    ...
    # closing database connection.
        ...
        return HttpResponse(json.dumps(response_data),content_type="application/json")
    return render(request,'reports/distance_report.html',{ 'form_params' : form_params})
##done
