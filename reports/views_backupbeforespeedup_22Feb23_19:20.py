...
def get_vehicle_travel_history_2(request,param):
    ...
    #return HttpResponse('Your parameter is  '+ route_code_id + ' '+ to_time)         #demo
...
def route_compliance(request):
    ...
    #print("Todate ", todate)    
        ...
        ward_id = str(fetch0[0])
        postgreSQL_select_Query1 = f"select id,plate_number,vehicle_type from swm.vehicles where ward_id = '{ward_id}' and vehicle_type IN ('Refuse_Compactor','Refuse_Mini_Compactor') and is_active = 't' order by id desc;"
        ...
        final_result = []
        Date_RouteCode = []
        vehicle_number_plate_array = []
        vehicle_number_plate_website_array = []
        vehicle_type_array2 = []
        ...
        for row1 in fetch1:
            ...
            postgreSQL_select_Query3 = f"select distinct t3.plate_number,t3.vehicle_type,t1.route_code,t1.route_name,t2.route_code_id from swm.route_allocation t1, swm.route_compliance t2, swm.vehicles t3 where t1.route_code <> '' and t1.shift = '{shift}' and t1.vehicle_id = '{vehicle_id}' and t1.ward_id = '{ward_id}' and t1.is_active = 't' and t2.is_active = 't' and t1.id = t2.route_code_id and t3.ward_id = t1.ward_id and t3.is_active = 't' and t3.ward_id = '{ward_id}' and t1.vehicle_id = t3.id and t3.vehicle_type IN ('Refuse_Compactor','Refuse_Mini_Compactor');"
            ...
            for row3 in fetch3:
                vehicle_number_plate = str(row3[0])
                vehicle_number_plate_array.append(vehicle_number_plate)
                vehicle_number_plate_website_array.append(vehicle_number_plate)

                vehicle_type2 = str(row3[1])
                vehicle_type_array2.append(vehicle_type2)
                vehicle_type_website_array2.append(vehicle_type2)


                route_code = str(row3[2])
                ...
                route_name = str(row3[3])
                route_name_array.append(route_name)
                route_name_website_array.append(route_name)
                
                route_code_id = str(row3[4])
                postgreSQL_select_Query4 = f"select bin_location,bin_code from swm.route_compliance where route_code_id = '{route_code_id}' and is_active = 't';"
                ...
                for row4 in fetch4:
                    bin_location = str(row4[0])
                    
                    bin_code = str(row4[1])

                    bin_location =  ", ".join(reversed(bin_location.split(",")))
                    bin_location = bin_location.replace(",", "")
                    bin_location = bin_location.split()
        ...
        #plate_number = "MH01ER0928"
        trial_res['plate_number'].append(vehicle_number_plate_website_array)
        #trial_res['data'] = 'TRIAL DATA111'
        
        
        trial_res['compliance_result_yes_no'].append(compliance_result_yes_no_website_array)

        trial_res['vehicle_type'].append(vehicle_type_website_array2)
    ...
    #return HttpResponse('Route Result is: ' + final_value)
    return render(request,'reports/route_compliance.html',{'trial_res' : trial_res})
...
def upload_bins(request):
    ...
    if request.method == 'POST':
        ...
        ws = wb.active
        excel_data = list()
        header= ['bin_code','bin_location','route_code']
        ...
        for row in ws.iter_rows(min_row=1,max_col=3):
            ...
            if row_content[2] and row_content[0] and row_content[1]:
                ...
                if not route_code:
                    print(f"route_code {row_content[2]} does not exists")
                    continue
                row_content[2] = route_code
                excel_data.append(dict(zip(header,row_content)))
